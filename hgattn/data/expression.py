...
"""
Dataset of the form: [PROGRAM] [INPUTS] [OUTPUTS], for example (with mod_val=113):

(((B + 6) * (B + B)) - 8) = 6 102 23 102 83 102 76 102 26 102 74 102
((C - 5) + C) = 86 23 56 54 41 107 103 77 96 88 36 74 58
((0 + A) * (7 + (A - B))) = 100 3 69 65 82 47 40 0 0 0 0 0
((6 + A) + (3 * B)) = 109 13 7 52 79 15 32 83 72 101 97 67
(A + ((A - 3) + 9)) = 107 107 107 107 107 107 107 107 107 107 107
(4 * ((A + C) + B)) = 59 93 31 54 34 24 109 103 40 104 84 8 106
(C + (5 * 6)) = 77 11 36 107 41 66 24 71 96 54 101 13 84
(B * (1 - 9)) = 45 29 92 107 55 48 12 68 17 21 90 58

[PROGRAM] is an arithmetic expression (see tools/arith.py) which accepts one or more
variable bindings A, B, etc.

[INPUTS] are n integers for an n-ary [PROGRAM].

[OUTPUTS] are generated by repeatedly applying [PROGRAM] with variables bound
as A=toks[t-1], B=toks[t-2], etc.

parameters specified in `InductiveOpts` 
"""
